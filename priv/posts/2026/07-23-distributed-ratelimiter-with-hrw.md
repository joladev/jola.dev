...
Traditionally this has been done using [ExHashRing](https://github.com/discord/ex_hash_ring) (the battle-tested [consistent hashing](https://en.wikipedia.org/wiki/Consistent_hashing) implementation for Elixir), but for clusters with less than 10 nodes there’s an alternative that’s potentially even faster and has slightly better distribution: [HRW](https://github.com/joladev/hrw) (highest random weight, also known as [rendezvous hashing](https://en.wikipedia.org/wiki/Rendezvous_hashing)). I wrote about this before [on this blog](https://jola.dev/posts/highest-random-weight-in-elixir). They both do the same thing, use ~~magic~~ math to associate any given key with a specific node, given a specific set of nodes. And both HRW and consistent hashing share the same incredibly important property: they cause minimal key re-assignment as the list of nodes changes. This means that if you auto-scale a node here and there, it won’t invalidate every key→node assignment, instead just a minimal subset.
